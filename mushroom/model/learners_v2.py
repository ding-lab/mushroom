...
import mushroom.data.multiplex_v2 as multiplex
...
["multiplex", "he", "visium"], got {self.dtype}')
    ...
    def embed_sections(self, device=None):
        ...
        cluster_probs = torch.zeros(n, num_patches, num_patches, self.sae.vq.codebook.shape[0])
        ...
        with torch.no_grad():
            for i, b in enumerate(self.inference_dl):
                ...
                # encoded_tokens, clusters, _, probs = self.sae.quantize(prequant_tokens)
        ...
        return recon_imgs, recon_embs, recon_embs_prequant, recon_cluster_ids, recon_cluster_probs